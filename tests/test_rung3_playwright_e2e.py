...
def test_rung3_ui_tier_consumption_locked_across_product_families():
    """قفلٌ صريحٌ عبر ثماني عائلات منتجاتٍ حقيقية دفعةً واحدة — بعد ترحيل
    القائمة الرسمية الكاملة وعكس التدفّق (الموجة ٤): **كل** منتجٍ يُظهِر
    صندوق حوار المرشّحين، لا «✓ صُنّف تلقائياً» إطلاقاً — القائمة المحلية لم
    تعد تقترح رمزاً بمعزلٍ عن كلود، فالتلقائي الصارم مستحيلٌ بنيوياً في
    بيئةٍ بلا مفتاح لأيّ منتج، لا فقط الغامض منه كما كان قبل عكس التدفّق.
    يغطّي نفس نقطة الاختناق (`ensureHs`) من مسار الاختيار عبر خطّاف
