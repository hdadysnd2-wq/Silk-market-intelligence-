...
def _apply_merchant_language(text: "str | None") -> "tuple[str, list]":
    ...
    (إن لم يشرحه الكاتب)، (٢) تسييق مبالغ الدولار الكبيرة بالريال (سعر الربط
    الرسمي 3.75). المسرد يُعاد **بنية** (لا نصّاً مُذيَّلاً) كي يعرضه كل
    مُصدِّر صراحةً — بما فيه تقرير العميل الذي يعيد ترتيب الأقسام. يشرح
    ويُسيّق فقط — لا يغيّر أي رقم ولا يخترع قيمة (عقد عدم الاختلاق)."""
    import re
    from silk_style_contract import GLOSSARY_ORDER, SAR_PEG
    s = str(text or "")
    if not s.strip():
        return s, []
    used: list = []
    ...
    def _sar(match) -> str:  # تسييق مبلغ الدولار بالريال عند سعر الربط
        whole, num_s, unit = match.group(0), match.group(1), match.group(2)
        converted = float(num_s.replace(",", "")) * SAR_PEG
        conv = f"{converted:.1f}".rstrip("0").rstrip(".")
        return f"{whole} (نحو {conv} {unit} ريال بسعر الربط 3.75)"
    # مليون/مليار فقط، ومرّة لكل مبلغ (تجنّب إعادة التسييق عبر lookahead للقوس).
    s = re.sub(r"(\d[\d,]*\.?\d*)\s*(مليون|مليار)\s+دولار(?!\s*\()", _sar, s)
