...
def deep_report(mission_reports: dict, analyst_summary: str, verdict: dict,
    ...
    parts.append(
        ...
        "الحقائق)؛ إن اختلطت عملات الصفوف فاذكر عملة كل صفّ في خليّته. (تسييق "
        "مبالغ الدولار الكبيرة بالريال عند سعر الربط الرسمي تضيفه طبقة العرض "
        "حتماً — لا تفعله يدوياً.) كل صف "
    ...
    # Q1 (تدقيق تسليم مقتطع، تمور/هولندا): حين يبلغ الكاتبُ السقفَ الصلب والنصُّ
    # ما زال مقتطعاً، كان «أوفى نص» يُسلَّم كما هو — منتهياً وسط جملة ("...وي")
    # وبأقسام ناقصة (المراجع علّق «أقسام مفقودة: الملاحق» ثم شُحن رغم ذلك). بدل
    # قطع خام وسط جملة نُلحِق فجوة معلنة صريحة — القارئ يعرف أن التقرير لم يكتمل
    # وأنّ إعادة التوليد (نداء واحد) تُكمِله، لا سطر مبتور صامت.
    if best and truncated:
        best = _mark_truncated_tail(best)
    return best or None


# علامة اكتمال معلنة تُلحَق بذيلٍ مقتطع — greppable وثابتة، يستهلكها العرض/التتبّع.
_TRUNCATION_MARKER = "⚠ القسم لم يكتمل"


def _mark_truncated_tail(text: str) -> str:
    """ألحِق فجوة اكتمال معلنة بنصٍّ بلغ سقف التوليد الصلب وهو مقتطع.

    يُكتشَف القطع وسط الجملة (آخر سطر غير فارغ لا ينتهي بعلامة إنهاء) وتُضاف
    الأقسام الناقصة صراحةً — فالقارئ يرى «لم يكتمل» بدل «...وي» المبتورة، ويعرف
    أن إعادة التوليد (نداء كاتب واحد) تُكمِله. حتمي، بلا نداء كلود إضافي.
    """
    body = (text or "").rstrip()
    if not body:
        return text
    missing = [s for s in _REPORT_SECTIONS
               if s not in re.findall(r"^##\s+\d+\.\s*(.+?)\s*$", body, re.M)]
    last_line = body.splitlines()[-1].strip()
    # ينتهي وسط جملة إن لم يُغلِق بعلامة إنهاء عربية/لاتينية أو رمز جدول/عنوان.
    mid_sentence = bool(last_line) and last_line[-1] not in ".؟!:|"
    if mid_sentence:
        body += " […]"   # يوضّح القطع بدل إيهام اكتمال الجملة
    note = (f"\n\n> {_TRUNCATION_MARKER} — بلغ توليد التقرير الحدّ الأقصى للطول "
            "قبل إتمامه، فالنصّ أعلاه مقتطع")
    if missing:
        note += " والأقسام التالية غير مكتملة: " + "، ".join(missing)
    note += (". أعِد توليد التقرير (نداء كاتب واحد، بلا إعادة بحث) لإكماله، "
             "أو ارفع SILK_MAX_TOKENS_CEILING.")
    return body + note
