...
| 2.1 Stale-data inline tag + `SILK_STALE_DATA_YEARS` | DONE-with-artifact (hermetic) — **redesigned to provenance-based (owner decision, LESSON #33)** | Staleness decided at the fact (`silk_staleness.fact_year`/`is_stale_fact`/`stale_fact_years` from `data_year`/`year=YYYY` marker/`retrieved_at`); tagged at the writer choke-point `silk_ai_judge._facts`; render tags the **stale-fact-year list** anywhere via `silk_render._tag_stale_years(text, stale_fact_years)` (regex demoted to conservative backstop) + `_stale_tag_misses` verification. Closes review findings #1/#2/#3/#5 in one design. `test_w2_1_*` (provenance, phrasing-independent, HS-2008-safe, food-word-safe, choke-point, verification). **Round-2 review fixes (owner decisions):** (#1) structured `year=` marker extended to Comtrade facts at the collector (`silk_llm_runtime._tool_comtrade_imports`), and `fact_year` `retrieved_at` fallback restricted to explicit year-end observation dates (`YYYY-12-31`) so a mid-year fetch stamp is never read as a data vintage — a 2018 Comtrade figure fetched today flags stale, a 2024 fresh; (#2) stale-tag verification miss routes to `silk_ops_log` (`stale_tag_missing`), never client `limits` (no internal-role leak); (#3) keyword backstop regained a right digit-boundary; (#4) era-suffix (م/هـ) consumed only at a word boundary (`2013مايو` keeps مايو intact); (#5/#6) import hoisted, redundant scan removed. |
