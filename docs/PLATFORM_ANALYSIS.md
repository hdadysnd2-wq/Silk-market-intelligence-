...
| 6 | **Quick snapshot** | Cheap "worth a full study?" probe: pricing_scout in budget-capped mode + Comtrade suppliers, cached | `POST /products/snapshot` `api.py:1413`; `silk_snapshot.py:83` | **Works-but-unverified-live** | `test_r4_product_snapshot.py` (offline declares gaps, cost precheck, cache). No live sample. |
