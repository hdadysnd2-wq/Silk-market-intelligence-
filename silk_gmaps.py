...
# التمور السعودية فعلاً (لا مبالغة في دلالة الرصد).
MAPS_DISCLAIMER = ("إدراج النشاط في خرائط قوقل يثبت وجوده وجهة اتصاله فقط، "
                   "لا أنه يستورد التمور السعودية — التحقق التجاري خطوة تالية.")
...
def extract_web_candidates(mission_reports: dict) -> list:
    ...
    والدمج مع رواد الخرائط. أسماء فقط، غير موثَّقة."""
    names: list = []
    ...
    for dp in (getattr(rep, "findings", None) or []):
        ...
        if isinstance(val, str) and val.strip():
            ...
            names.append(str(val["name"]).strip())
    return names
