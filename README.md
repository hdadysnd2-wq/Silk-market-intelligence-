...
| `silk_importers_agent.py` | **المجموعة ج · Group C**: مستوردون/موزّعون محتملون عبر **بحث ويب مجاني** (النسخة المدفوعة الأدقّ Volza حصراً في زر «تعميق التحليل»). |
...
- `with_competition` (**المجموعة ج · Group C**) يُرفق `row['competitors_web']` و`row['importers']` (بحث ويب مجاني؛ Volza المدفوع حصراً عبر `/deepen`) و`row['distribution_channels']` و`row['ecommerce']` (بحث ويب ديناميكي، يتطلب `SEARCH_API_KEY`) و`row['bestsellers']` (ترتيب الأكثر مبيعاً عبر مُشغّل Apify مرخّص، يتطلب `APIFY_API_TOKEN` + `APIFY_BESTSELLERS_ACTOR` — لا سكرابينغ مباشر). الكل **إضافي** لا يغيّر `total_score`، ويتدهور بأمان بلا مفاتيح/شبكة.
...
- **ضبط تكلفة كلود (سقف صلب قبل الإرسال)** — طبقتا كلود (`with_ai`, `with_synthesis`) هما جوهر المنصة وتعملان في المسار العادي (ليستا محجوبتين خلف زر التعميق)، لكنهما مقيّدتان بسقف تكلفة صلب: `SILK_AI_TOKEN_CAP` (افتراضي 50000 توكن لكل تحليل، إدخال تقديري + إخراج). قبل كل نداء يتحقّق `silk_ai_judge._call`: إن كان النداء التالي سيتجاوز السقف لا يُرسله ويعيد `None` مع سطر تحذير واضح، فتتدهور المنصة إلى اللجنة الحتمية بلا اختلاق ولا فاتورة صامتة. يُصفَّر السقف في بداية كل تحليل (`reset_budget()`)؛ اضبطه 0 لتعطيله.
  - **القطع مرئيّ لا صامت**: عند بلوغ السقف منتصف التحليل، كل سوق مقطوع يحمل مؤشّراً صريحاً `jury.ai_skipped` (و`synthesis_skipped`) بالسبب `cost_cap`، ويُرفَق ملخّص على مستوى النتيجة `result.ai_cost` = `{cap, spent, remaining, cap_hit, blocked_calls, markets_ai, markets_cap_skipped, note}` فيراه المستخدم بوضوح («سوقان اكتملا، واحد توقّف بالسقف») بدل فجوة صامتة. تعرضهما الواجهة كشريط تنبيه علوي + شارة لكل سوق.
- **الأدوات المدفوعة = زر التعميق فقط** — Google Maps (Places)، Volza، explee، الأكثر-مبيعاً (Apify)، وأسعار التجزئة المُهيكلة (SerpApi) **لا تُستدعى في `/analyze` أبداً** ولو كان المفتاح محفوظاً؛ تعمل يدويًا عبر `/deepen` للأسواق الثلاثة الأعلى فقط. في المسار العادي: المستوردون وإشارات سعر التجزئة (`with_localprice`) بحث ويب مجاني.
